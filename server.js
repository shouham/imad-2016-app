...
function createTemplate(titles,dates,headings,contents){
    ...
    var htmlTemplate = '<html><head><title>title</title><meta name="viewport" content="width=device-width,initial-scale=1"/>        <link href="/ui/style.css" rel="stylesheet"/></head><body><div class="container"><div><a href="/">Home</a></div><hr/><h3>{{headings}}</h3><div>${date}</div><div>${content}</div></div><script src="angular/angular.min.js"></script></body></html>';
    return htmlTemplate;
}
